...
class raspinfo
{
	...
	public function getInfo(){
		$aLinux_dist = explode( ':', shell_exec('lsb_release -i'));
		$aLinux_vers = explode( ':', shell_exec('lsb_release -r'));
		$aLinux_code = explode( ':', shell_exec('lsb_release -c'));
		$aLinux_desc = explode( ':', shell_exec('lsb_release -d'));
		
		return array(
			'distribution' => $aLinux_dist[1], 
			'version' => $aLinux_vers[1], 
			'codename' => $aLinux_code[1], 
			'description' => $aLinux_desc[1],
		);
	}
	...
	public function getCpuData(){
		...
		return array(
			'model' => $sModel[1], 
			'clockspeed' => $sClock/1000, 
			'corecount' => substr($aCores[1], 0, 3), 
			'coretemp' => substr($sTemp/1000, 0, 4),
		);
